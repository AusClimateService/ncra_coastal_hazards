...
* domain - [GESLAv3](https://gesla787883612.wordpress.com/), [ACSSH](https://data.csiro.au/collection/csiro:29013?q=coast%20hindcast&_st=keyword&_str=4&_si=2) or 
[CCHaPS](https://doi.org/10.25919/6tbn-px91)
...
* SWL_AEPPC - Still Water Level AEP (including SLR) [m]
* MTWL_AEPPC - Mean Total Water Level AEP (including SLR and wave setup) [m]
...
cchaps = cchaps[which(!is.na(cchaps$SWLlam) & !is.na(cchaps$MTWLlam)),]
which(is.na(cchaps$SWLlam))
which(is.na(cchaps$MTWLlam))

cchaps$beta2 = cchaps$beta
cchaps$beta2[cchaps$beta2 == 0.087] = NA

tg = vect("gesla3_fgumbel_shp")
tg = tg[tg$nCompleteY >= 20]

tg = crop(tg,ext(cchaps))

#Align the cchaps 1 year ARI (63% AEP) location parameter to the nearest tide gauge value following O'Grady et al 2019 and Haigh et al. 2014.
crs(cchaps) = crs(tg)
nb = nearby(cchaps,tg)
...
```
```{r, include=FALSE}
plot_with_stats <- function(x, y_roms, y_cchaps,
                            xlab = "Observed (TG RL)",
                            ylab = "Modelled RL",
                            main = "TG vs ROMS & CCHaPS",
                            roms_name = "ROMS",
                            cchaps_name = "CCHaPS",
                            legend_loc = "topleft") {
  # Helper to compute stats on complete pairs
  comp_stats <- function(x, y) {
    idx <- is.finite(x) & is.finite(y)
    x2 <- x[idx]; y2 <- y[idx]
    r    <- if (length(x2) > 1) cor(x2, y2) else NA_real_
    bias  <- mean(y2 - x2)
    rmse  <- sqrt(mean((y2 - x2)^2))
    list(r = r, bias = bias, rmse = rmse, n = length(x2))
  }

  s_roms   <- comp_stats(x, y_roms)
  s_cchaps <- comp_stats(x, y_cchaps)

  # Base plot
  plot(x, y_roms,
       xlab = xlab, ylab = ylab, main = main,
       pch = 1, col = "black",asp=1)
  points(x, y_cchaps, pch = 1, col = "red")

  # 1:1 reference line
  abline(0, 1, lty = 2, col = "grey50")

  # Legend text (two blocks: one per model)
  leg <- c(
    roms_name,
    sprintf("  R = %.2f | Bias = %.2f | RMSE = %.3f (n=%d)",
            s_roms$r, s_roms$bias, s_roms$rmse, s_roms$n),
    cchaps_name,
    sprintf("  R = %.2f | Bias = %.2f | RMSE = %.3f (n=%d)",
            s_cchaps$r, s_cchaps$bias, s_cchaps$rmse, s_cchaps$n)
  )

  legend(legend_loc,
         legend = leg,
         col = c("black", NA, "red", NA),
         pch = c(1, NA, 1, NA),
         bty = "n")
}
...
Compare the ROMS and CCHaPS Return levels to the tide gauge (correcting for the location parameter acording to Haigh et al 2014 and O'Grady et al 2019) for a single AEP return level for each GESLA tide gauges with at least 20 years (118 gauges).



```{r, fig.width=7, fig.height=7}
bn = nearby(tg,cchaps)
tg$cchaps_mu = cchaps$SWLmu[bn[,2]]
tg$cchaps_lam = cchaps$SWLlam[bn[,2]]

roms = vect("ROMS_gumbel_parameters_SWLandMTWL_Australia")
crs(roms) = crs(tg)
bn = nearby(tg,roms)
tg$roms_mu = roms$SWLmu[bn[,2]]
tg$roms_lam = roms$SWLlam[bn[,2]]

AEPv = 0.02
tg$RL = evd::qgev(1-AEPv,loc = tg$mle_1,scale = tg$mle_2,shape = 0)
tg$roms_RL = evd::qgev(1-AEPv,loc = tg$mle_1,scale = tg$roms_lam,shape = 0)
tg$cchaps_RL = evd::qgev(1-AEPv,loc = tg$mle_1,scale = tg$cchaps_lam,shape = 0)

plot_with_stats(tg$RL, tg$roms_RL, tg$cchaps_RL, main = paste("TG vs ROMS & CCHaPS",AEPv,"AEP"))


```

CCHaPS slightly outperforms ROMS by halving the mean bias (–0.05 vs –0.11), while achieving a nearly identical RMSE (0.156 vs 0.157). Both models explain ~99% of the variance, but CCHaPS reduces systematic offset against TG observations, indicating a modest though meaningful improvement in accuracy.

```{r}
#Define a region for each point
#regions = project(vect("Coastal_Waters_areas_(AMB2020)"),cchaps)
#regions$state = c("VIC","TAS","QLD","NT","WA","SA","NSW")

regionsb = project(vect("NCRA_regions_coastal_waters_GDA94"),cchaps)
regionsb$short_name = regionsb$NCRA
regionsc = project(vect("ncra_regions"),cchaps)
...
ints = relate(cchaps,regions,"intersects",pairs=TRUE)
cchaps$state = NA
cchaps$state[ints[,1]] = regions$state[ints[,2]]
plot(cchaps,"state")
```
...
cchaps$id = 1:length(cchaps)
cchapsp = project(cchaps,lgas)
```
...
```{r}
require(sf)
nearest_lgas <- function(cchapsp, lgas) {
  # Convert SpatVector objects to sf objects
  cchapsp_sf <- st_as_sf(cchapsp)
  lgas_sf <- st_as_sf(lgas)
  
  # Find the nearest LGAs for each cchapsp feature
  nearest_indices <- st_nearest_feature(cchapsp_sf, lgas_sf)
...
result <- nearest_lgas(cchapsp, lgas)
cchaps$lga = result$LGA_NAME24
plot(cchaps,"lga")
lines(lgas)
#length(unique(cchaps$lga))

#writeVector(cchaps,"cchaps/cchaps.shp",overwrite=TRUE)
```
...
cchaps$SWL_RL = return_level(cchaps$muOff,cchaps$SWLlam,AEP)
#storm tide plus wave setup
cchaps$MTWL_RL = return_level(cchaps$muOff-cchaps$SWLmu+cchaps$MTWLmu,cchaps$MTWLlam,AEP)
...
SWL_rls = t(sapply(1:length(cchaps), function(x) t(gumbel_mleAndConfIntvals(loc = cchaps$muOff[x],scale = cchaps$SWLlam[x],ARI = 1/AEP,nMonteCarloSims = 100))))
MTWL_rls = t(sapply(1:length(cchaps), function(x) t(gumbel_mleAndConfIntvals(loc = cchaps$muOff[x]-cchaps$SWLmu[x]+cchaps$MTWLmu[x],scale = cchaps$MTWLlam[x],ARI = 1/AEP,nMonteCarloSims = 100))))

gg = terra::geom(cchaps)
out = data.frame(state = cchaps$state,lga = cchaps$lga,longitude = gg[,3],latitude = gg[,4],
...
write.csv(saout,paste0("data/SA_cchaps_WWM_OGRADY_ET_AL_2019_",AEP*100,"pcAEP.csv"),row.names = FALSE)

tgsa = crop(tg,ext(cchaps[cchaps$state == "SA"]))

tg_SWL_rls = t(sapply(1:length(tgsa), function(x) t(gumbel_mleAndConfIntvals(loc = tgsa$mle_1[x],scale = tgsa$mle_2[x],ARI = 1/AEP,nMonteCarloSims = 100))))
...
write.csv(tgout,paste0("data/SA_GESLA3_TIDEGAUGES_OGRADY_ET_AL_2019_",AEP*100,"pcAEP.csv"),row.names = FALSE)
...
ats = seq(0,7,0.5)
plotit = function(){
  plot(cchaps,"SWL_RL",breaks=ats,range = range(ats), main = paste0(AEP*100,"% AEP extreme SWL (reletive to MSL of that year)"),type = "continuous",col = colzRamp(length(ats)-1))
...
plotit <- function(SLR) {
  ...
  dat <- plotMF(v = cchaps, att = "SWL_MFSLR_50", ats = ats, SLR = SLR, 
  ...
  out <- data.frame(
    ...
    latitude = ge[, 4], 
    SLR = dat$SLR, 
    SWL_AEPPC = round(dat$SWL_RL + dat$SLR, 2), 
    MTWL_AEPPC = round(dat$MTWL_RL + dat$SLR, 2), 
  ...
  write.csv(out, file = paste0(outdir, "../data/MFSLR_CCHaPS_MMALL_SLR", SLR * 100, "cm.csv"), row.names = FALSE)
  
  dattg <- plotMF(v=tg, att="SWL_MFSLR_50", ats = ats, SLR = SLR, cex = 1.5, add = TRUE, legend = FALSE)
  plot(tg, cex = 1.5, SLR = SLR, pch = 1, add = TRUE)
  plotMF(v = cchaps, att = "MTWL_MFSLR_50", ats = ats, SLR = SLR, 
  ...
  outtg <- data.frame(
    ...
    latitude = ge[, 4], 
    SLR = dattg$SLR, 
    SWL_AEPPC = round(dattg$SWL_RL + dattg$SLR, 2), 
...
for (SLR in SLRs) {
  pdf(paste0(outdir, "MFSLR_CCHaPS_MM50_SLR_", SLR * 100, "cm.pdf"), width = 14, height = 5)
  plotit(SLR=SLR)
  dev.off()
}
...
infiles <- paste0("data/MFSLR_CCHaPS_MMALL_SLR", SLRs * 100, "cm.csv")
