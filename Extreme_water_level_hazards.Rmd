...
SWL_rls = t(sapply(1:length(roms), function(x) t(gumbel_mleAndConfIntvals(loc = roms$muOff[x],scale = roms$SWLlam[x],ARI = 1/AEP,nMonteCarloSims = 100))))
MTWL_rls = t(sapply(1:length(roms), function(x) t(gumbel_mleAndConfIntvals(loc = roms$muOff[x]-roms$SWLmu[x]+roms$MTWLmu[x],scale = roms$MTWLlam[x],ARI = 1/AEP,nMonteCarloSims = 100))))
...
write.csv(saout,paste0("data/SA_ROMS_CAWCR_OGRADY_ET_AL_2019_",AEP*100,"pcAEP.csv"),row.names = FALSE)
...
ats = seq(0,7,0.5)
plotit = function(){
  plot(roms,"SWL_RL",breaks=ats,range = range(ats), main = paste0(AEP*100,"% AEP extreme SWL (reletive to MSL of that year)"),type = "continuous",col = colzRamp(length(ats)-1))
...
ats = seq(0,7,0.5)
plotit = function() plot(roms,"MTWL_RL",breaks=ats,range = range(ats), main = paste0(AEP*100,"% AEP extreme MTWL (reletive to MSL of that year)"),type = "continuous",col = colzRamp(length(ats)-1))
