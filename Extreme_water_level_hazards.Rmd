...
plot_with_stats <- function(x, y_roms, y_cchaps,
  ...
  comp_stats <- function(x, y) {
    ...
    x2 <- x[idx]; y2 <- y[idx]
    r2    <- if (length(x2) > 1) cor(x2, y2)^2 else NA_real_
    bias  <- mean(y2 - x2)
    rmse  <- sqrt(mean((y2 - x2)^2))
    list(r = r2, bias = bias, rmse = rmse, n = length(x2))
  }
  ...
  leg <- c(
    roms_name,
    sprintf("  R2 = %.2f | Bias = %.2f | RMSE = %.3f (n=%d)",
            s_roms$r2, s_roms$bias, s_roms$rmse, s_roms$n),
    cchaps_name,
    sprintf("  R2 = %.2f | Bias = %.2f | RMSE = %.3f (n=%d)",
            s_cchaps$r2, s_cchaps$bias, s_cchaps$rmse, s_cchaps$n)
  )
